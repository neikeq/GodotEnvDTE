...
namespace GodotEnvDTE
{
    class Program
    {
        ...
        {
            Console.WriteLine("Opens the file(s) in a Visual Studio instance that is editing the solution.");
            Console.WriteLine("If an existing instance for the solution is not found, a new one is created.");
            Console.WriteLine();
            Console.WriteLine("Usage:");
            Console.WriteLine(@"  GodotEnvDTE solution [file[:line[:col]]...]");
            Console.WriteLine();
        ...
        static int Main(string[] args)
        {
            ...
            DTE dte = FindInstanceEditingSolution(solutionFile);

            if (dte == null)
            {
                // Open a new instance

                Type visualStudioDTEType = Type.GetTypeFromProgID("VisualStudio.DTE.15.0");
                dte = Activator.CreateInstance(visualStudioDTEType) as DTE;
            ...
            try
            {
                ...
                for (int i = 1; i < args.Length; i++)
                {
                    ...
                    string[] fileArgumentParts = args[i].Split(new char[] { ':' });
        ...
        static DTE FindInstanceEditingSolution(string solutionPath)
        {
            ...
            try
            {
                ...
                while (ppenumMoniker.Next(1, moniker, IntPtr.Zero) == 0)
                {
                    ...
                    if (!Regex.IsMatch(ppszDisplayName, "!VisualStudio.DTE.15.0:[0-9]"))
                        continue;
        ...
        public class MessageFilter : IOleMessageFilter
        {
            ...
            static IOleMessageFilter oldFilter = null;

            // Start the filter
            public static void Register()
            {
                IOleMessageFilter newFilter = new MessageFilter();
                int ret = CoRegisterMessageFilter(newFilter, out oldFilter);
                if (ret != 0)
            ...
            {
                int ret = CoRegisterMessageFilter(oldFilter, out _);
                if (ret != 0)
                    Console.Error.WriteLine($"CoRegisterMessageFilter failed with error code: {ret}");
            }
